...
const authorsWitoutDoublon = ref(booksStore.authors.filter(
  (author, index, self) =>
  ...
  <Menu as="div" class="relative inline-block text-left">
    ...
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
      ...
      <MenuItems
        ...
        <div v-for="g in filteredAuthors" :key="g.name" class="space-y-2">

          <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
            <input type="checkbox" :value="g.idAuthor" v-model="selectedAuthors" />
            {{ g.name }}
            // checker select auteur 1 seul partir du scope et revenir (ya tt les auters selecvt)
